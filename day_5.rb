input = File.open("day_5_input.txt").read

seeds, *groups = input.split(/\n\n/)
_, *seeds = seeds.split.map(&:to_i)

groups = groups.map do |group|
  _, *mappings = group.split("\n")
  mappings.map { |line| line.split.map(&:to_i) }
end

def find(group, seed)
  group.each do |ds, src, len|
